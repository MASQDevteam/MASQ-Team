pageextension 70127 "PO Subform e xtension" extends "Purchase Order Subform"
{
    layout
    {
        ...
        modify(Type)
        {
            Editable = (Rec."MASQ Sales Order No." = '') OR (CanEditPO_SO_Lines);
        }
        modify("No.")
        {
            Editable = (Rec."MASQ Sales Order No." = '') OR (CanEditPO_SO_Lines);
            ...
            begin
                ...
                IF (NOT CanEditPO_SO_Lines) then
                    if REc."MASQ Sales Order Line No." <> 0 then
                        Error(Text002, Rec.FieldCaption(Quantity), Rec."MASQ Sales Order No.", Rec."MASQ Sales Order Line No.");
                    ...
                    IF Rec.Quantity <> 0 then begin
                        IF Rec.Quantity <> SalesLine.Quantity then begin
                            IF Confirm(StrSubstNo('Warninig! the quantity in the sales %1 line %2 is different from the purchase line. Do you want to update the Quantity on the Purchase Line too?',
                                  Rec."MASQ Sales Order No.", Rec."MASQ Sales Order Line No.")) then begin
                                //ADDED on 13/01/2025
    ...
    var
        ...
        CurrentIndex, TargetIndex, Count, i : Integer;
        LineNoStep: Integer;
    begin
        ...
        //  Rearrange order in memory
        LineNos.RemoveAt(CurrentIndex);
        LineNos.Insert(TargetIndex, LineToMoveNo);

        //  Use temporary table to store reordered lines
        ...
        for i := 1 to Count do begin
            ...
            PurchLines.SetRange("Document Type", Rec."Document Type");
            PurchLines.SetRange("Document No.", Rec."Document No.");
            PurchLines.SetRange("Line No.", LineNos.Get(i));
            if PurchLines.FindFirst() then begin
                TempPurchLines.Init();
                TempPurchLines.TransferFields(PurchLines, true);
                TempPurchLines."Line No." := i * LineNoStep;
                TempPurchLines.Insert();
            end;
        end;

        // Delete all original lines for this document
        PurchLines.Reset();
        PurchLines.SetRange("Document Type", Rec."Document Type");
        PurchLines.SetRange("Document No.", Rec."Document No.");
        ...
        //  Reinsert from temporary in correct order
        if TempPurchLines.FindSet() then
            repeat
                PurchLines.Init();
                PurchLines.TransferFields(TempPurchLines, true);
                PurchLines.Insert();
            until TempPurchLines.Next() = 0;
